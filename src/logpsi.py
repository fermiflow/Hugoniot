...
def make_logpsi(network_wfn, lcao_orbitals):
    """
        Make the logpsi electron wavefunction.
        lcao_orbitals is a static function generated by lcao.
        kpt is fixed at gamma point.
    """
    
    def logpsi(x, params, s, state_idx, mo_coeff):
        """
        ...
        INPUT:
            ...
            s: (n, dim)
            state_idx: (n,)
            mo_coeff: coefficient of lcao orbitals on atomic orbitals (n_ao, n_mo)
        OUTPUT:
        ...
        # log(det)
        z = network_wfn.apply(params, None, jnp.concatenate([s, x]))
        D_up, D_dn = lcao_orbitals(s, z, mo_coeff, state_idx)
        phase, logabsdet = logdet_matmul([D_up[None, :, :], D_dn[None, :, :]])
        log_phi = logabsdet + jnp.log(phase + 0.j)

        # jacobi
        x_flatten = x.reshape(-1)
        flow_flatten = lambda x: network_wfn.apply(params, None, jnp.concatenate([s, x.reshape(n, dim)])).reshape(-1)
...
def make_logpsi2(logpsi):
    ...
    def logpsi2(x, params, s, state_idx, mo_coeff):
        """ logp = logpsi + logpsi* = 2 Re logpsi """
        return 2 * logpsi(x, params, s, state_idx, mo_coeff)[0]
...
def make_logpsi_grad_laplacian(logpsi, forloop=True, hutchinson=False):
    ...
    @partial(jax.vmap, in_axes=(0, None, 0, 0, 0), out_axes=0)
    def logpsi_vmapped(x, params, s, state_idx, mo_coeff):
        logpsix = logpsi(x, params, s, state_idx, mo_coeff)
        return logpsix[0] + 1j * logpsix[1]

    @partial(jax.vmap, in_axes=(0, None, 0, 0, 0, None), out_axes=0)
    def logpsi_grad_laplacian(x, params, s, state_idx, mo_coeff, key):
        """
        ...
        grad = jax.jacrev(logpsi)(x, params, s, state_idx, mo_coeff)
        grad = grad[0] + 1j * grad[1]
        print("Computed gradient.")

        n, dim = x.shape
        x_flatten = x.reshape(-1)
        grad_logpsi = jax.jacrev(lambda x: logpsi(x.reshape(n, dim), params, s, state_idx, mo_coeff))
    ...
    def logpsi_grad_laplacian_hutchinson(x, params, s, state_idx, mo_coeff, key):

        v = jax.random.normal(key, x.shape)

        @partial(jax.vmap, in_axes=(0, None, 0, 0, 0), out_axes=0)
        def logpsi_grad_random_laplacian(x, params, s, state_idx, mo_coeff, v):
            """
            ...
            grad, hvp = jax.jvp( jax.jacrev(lambda x: logpsi(x, params, s, state_idx, mo_coeff)), (x,), (v,) )
        ...
        return logpsi_grad_laplacian(x, params, s, state_idx, mo_coeff, v)

    return logpsi_vmapped, (logpsi_grad_laplacian_hutchinson if hutchinson else logpsi_grad_laplacian)


def make_quantum_score(logpsi):

    @partial(jax.vmap, in_axes=(0, None, 0, 0, 0), out_axes=0)
    def quantum_score_fn(x, params, s, state_idx, mo_coeff):
        """
        ...
        """
        grad_params = jax.jacrev(logpsi, argnums=1)(x, params, s, state_idx, mo_coeff)
        return jax.tree_util.tree_map(lambda jac: jac[0] + 1j * jac[1], grad_params)
